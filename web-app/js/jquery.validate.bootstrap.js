(function() {
	...
	$.validator.setDefaults({
		...
		highlight : function(element, errorClass, validClass) {
			var $obj;
			$obj = element.type === 'radio' ? this.findByName(element.name)
					: $(element);
			return $obj.parents('div.control-group').removeClass(validClass)
					.addClass(errorClass);
		},
		unhighlight : function(element, errorClass, validClass) {
			...
			return $obj.parents('div.control-group').removeClass(errorClass)
					.addClass(validClass);
		},
	...
	$.validator.messages = ({
		...
		maxlength : jQuery.validator
				.format('needs to be more than {0} characters'),
		minlength : jQuery.validator
				.format('needs to be at least {0} characters'),
		rangelength : jQuery.validator
				.format('needs to be a value between {0} and {1} characters long'),
		range : jQuery.validator
				.format('needs to be a value between {0} and {1}'),
		max : jQuery.validator
				.format('needs to be a value less than or equal to {0}'),
		min : jQuery.validator
				.format('needs to be a value greater than or equal to {0}')
	});
	...
	$.validator.prototype.optional = function(element) {
		return $(element).hasClass('optional')
				|| (!$.validator.methods.required.call(this, $
	...
	$.validator.prototype.showLabel = function(element, message) {
		...
		if (label.length) {
			...
			label = $('<' + this.settings.errorElement + '/>').attr({
				...
				'generated' : true
			}).addClass(this.settings.errorClass).html(message || '').addClass(
					'help-inline');
